...
public class HistoryAdapter extends RecyclerView.Adapter<HistoryAdapter.MyViewHolder> {
    ...
    private List<LinkEntity> historyList;
    private Context mContext;
    private LayoutInflater inflater;
    private HistoryAdapter.OnItemClickListener mOnItemClickListener;

    public HistoryAdapter(Context context, List<LinkEntity> datas) {
    ...
    public void onBindViewHolder(HistoryAdapter.MyViewHolder holder, final int position) {
        String title = historyList.get(position).getLinkTitle();
        String url = historyList.get(position).getLinkUrl();
        if (title.length() > 40) {
